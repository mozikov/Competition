...
class ConicDataset(Dataset):
    ...
    def __init__(self, image_path, mask_path, transform, copypaste, copy_level):
...
#         self.source = source
        ...
        with open('insufficient_class_alldict/insufficient_class_alldict_FinalSplit.pickle','rb') as temp:
...
#             self.insufficient_class_alldict_pannuke = pickle.load(temp)             
    ...
    def __getitem__(self, idx):
        ...
        mask = self.mask_path[idx].astype('uint8')
...
run_info = '{}_Unet++_Likebaseline_copypaste_candidate_{}_allclass_focalloss{}_FinalSplit_scheduler50_re6'.format(today, copy_level, gamma)
...
log.info("Copy Paste: {}".format(copy_level))
...
train_dataset = ConicDataset(train_images, train_masks, transform=True, copypaste=True, copy_level = copy_level)
train_dataloader = DataLoader(train_dataset, batch_size=8, num_workers=12, shuffle=True)

test_dataset = ConicDataset(test_images, test_masks, transform=False, copypaste=False, copy_level = None)
...
weight = torch.FloatTensor([1, 1.2, 1, 1, 1, 1.2, 1]).to(device)

criterion_CE_weight = nn.CrossEntropyLoss(weight=weight).to(device)
...
pred_final = np.zeros(shape=(len(test_dataset), train_images.shape[1], train_images.shape[2], 2))
gt_final = np.zeros(shape=(len(test_dataset), train_images.shape[1], train_images.shape[2], 2))
...
#             classification_celoss = criterion_CE_weight(output_classification, mask[:, 0, :, :])
    ...
    if (epoch % 1 == 0) & (epoch > 44):
        ...
        torch.save(model.state_dict(), 'checkpoint/{}/Epoch {} Loss : {:.4f}, Dice : {:.4f}, PQ : {:.4f}, DQ : {:.4f}, SQ :{:.4f}, Multi_PQ+ : {:.4f}.pth'.format(run_info, epoch,
                                                                                           sum(loss_total) / len(loss_total),
                                                                                           sum(dice_total) / len(dice_total),
                                                                                           result['pq'][0],
                                                                                           result['DQ'][0].mean(),
